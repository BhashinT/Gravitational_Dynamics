...
! output format is time followed by 6 values for each particle
!     write(fmt,'("(f12.6,",i0,"es16.8)")') 9*np
      write(fmt,'("(f12.6,",i0,"es16.8)")') 6*np
...
!     write(*,fmt) t,(sys(i)%pos,sys(i)%vel,sys(i)%mass,sys(i)%Epot,sys(i)%Ekin,i=1,np)
      write(*,fmt) t,(sys(i)%pos,sys(i)%vel,i=1,np)
